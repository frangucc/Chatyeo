...
image_search (Topics, State) ->
    lists:mapfoldl(fun(Topic, Row) -> 
                           Url = lists:flatten(io_lib:format("http://~s/~s?v=~s&q=~s", [?GOOGLE_URL, ?IMAGE_TYPE, ?VERSION, ibrowse_lib:url_encode(Topic)])),
                           {ok, Results, _} = rfc4627:decode(restful_querying:http_g_request(Url)),
                           image_parse (Results, Row, State),
                           {[], Row+1}
                   end, 1, Topics).

image_parse({obj,[{"responseData", {obj,[{"results", Results}, _]}}, _, _]}, Row, State) ->
    lists:mapfoldl(fun(X, Depth) ->
                           {ok, URL} = rfc4627:get_field(X, "url"),
                           {ok, Title} = rfc4627:get_field(X, "title"),
                           Json = rfc4627:encode(bot_utils:make_json_google_images(Row, Depth, URL, URL, Title)),
