...
main() ->
    ...
    case wf:user() of
        undefined -> Header = nitrogen:get_wwwroot()++"/new_user_template.html";
        _ -> Header = nitrogen:get_wwwroot()++"/user_template.html"
    end,
...
event (register) ->
    ...
    if
        ...
        true ->
            case jabber_utils:register_user (hd(wf:q(username)), "Mr. None", hd(wf:q(email_address)), Pass) of
                ok ->            
                    wf:redirect("login");
                {aborted, Reason} ->
