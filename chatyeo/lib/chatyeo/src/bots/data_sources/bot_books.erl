...
handle_cast({message_from, Msg}, State) ->
    ...
    case Type of
        bot_topic ->
            ...
            lists:mapfoldl (fun({Title, Author, ImgUrl, Desc}, Row) ->
                                    Depth = 1,
                                    X = make_json_books (Row, Depth, Title, Author, ImgUrl, Desc),
...
%%% Internal functions
%%--------------------------------------------------------------------
make_json_books (Row, Depth, Title, Author, ImgUrl, Desc) ->
    Time = bot_utils:get_unix_timestamp(),    
    {obj, [{'key', [Time, <<"\\u9999">>]}, {'Function', ?BOOK_JS}, {'Row', Row}, {'Depth', Depth},{'Title', list_to_binary(Title)}, {'Author', list_to_binary(Author)}, {'ImgUrl', list_to_binary(ImgUrl)}, {'Desc', list_to_binary(Desc)}]}.
