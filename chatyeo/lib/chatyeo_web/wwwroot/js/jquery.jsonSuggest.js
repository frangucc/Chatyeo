    ...
    return this.each(function() {
      ...
      function buildResults(resultObjects, sFilterTxt) {
        ...
        for (i = 0; i < resultObjects.length; i += 1) {
          ...
          if (typeof resultObjects[i].nameTag === 'string') {
            var nameTag = '<b>'+ resultObjects[i].nameTag +':</b>';
          } else {
            var nameTag = '';
          }

          $(item).append('<p class="text">'+ nameTag +' '+ text + '</p>');
        ...
        if (settings.newtopic === true) {
          var item=$('<div id="topic_create" style="display:block !important;"><a id="topic_create_url" href="javascript:;">Start a new topic</a></div>');
          $(results).append(item);
          if(typeof settings.newtopic_callback === 'function') {
            $("#topic_create_url").click(function() {
              $(results).html('').hide();
              settings.newtopic_callback(sFilterTxt)
            });
          }
        }
      ...
      function runSuggest(e) {
        ...
        if (settings.ajaxResults === true) {
            ...
            if (settings.needEval === true) {
                resultObjects = eval('(' + resultObjects + ')');
            }
      ...
      else
      {
        $(results).addClass('jsonSuggestResults').
        css({
        'bottom': (+obj.position().top + obj.height() +60) + 'px',
        'left': (+obj.position().left + 11)+ 'px',
        'width': obj.width() + 'px'
      }).hide();
